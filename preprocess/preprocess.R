...
#clean the corpus text and save it
clean <- function(data) {
	if (is.null(data$txt)) {
		data$txt <- readRDS(sprintf("%s/%s.rds", data.raw.dir, data$corpus))
	}
	data$lines <- length(data$txt)
	if(is.null(data$ids)) {
		data$ids <- initIds(data)
	}
	t <- system.time(data <- cleanCorpus(data))
	if (!file.exists("data/clean")) {
		dir.create("data/clean", showWarnings = FALSE)
	}
	saveRDS(data$clean, sprintf("%s/%s.%s.rds", data.clean.dir, data$corpus, data$suffix))
	printf("text preprocessing completed in %.3f s\n", t[3])
	data
}


#split the corpus in chunks and clean it, chunk by chunk
cleanCorpus <- function(data) {
	ids <- data$ids
	txt <- data$txt
	stemming <- data$stemming
	for(n in 1:(nrow(ids) - 1) ) {
		t0 = Sys.time()
		txt[ids[n,]] <- cleanText(txt[ids[n,]], stemming)
		printf("chunk %d completed in %.3f s\n", n, Sys.time() - t0)
	}
	t0 = Sys.time()
	lastRowIds <- ids[n + 1,]
	lastRowIds <- lastRowIds[!is.na(lastRowIds)]
	txt[lastRowIds] <- cleanText(txt[lastRowIds], stemming)
	printf("chunk %d completed in %.3f s\n", n + 1, Sys.time() - t0)
	data$clean <- txt
	data
}
